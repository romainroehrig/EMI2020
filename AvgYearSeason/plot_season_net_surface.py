...
def generate_season_net_surface(fin1, fin2, fin3, fin4, fin5, fin6, fin7, fin8, data_type, vmin, vmax):
...
# Temporal mean - season
    ...
    for i in range(10):
        for j in range(4):
            ...
            data_5[j] = data_5[j] + data5[335+3*j+12*i] + data5[336+3*j+12*i] + data5[337+3*j+12*i]
            data_6[j] = data_6[j] + data6[335+3*j+12*i] + data6[336+3*j+12*i] + data6[337+3*j+12*i]
            data_7[j] = data_7[j] + data7[335+3*j+12*i] + data7[336+3*j+12*i] + data7[337+3*j+12*i]
            data_8[j] = data_8[j] + data8[335+3*j+12*i] + data8[336+3*j+12*i] + data8[337+3*j+12*i]
    ...
    for j in range(4):
        ...
        data_5_moy[j]= data_5[j]/30
        data_6_moy[j]= data_6[j]/30
        data_7_moy[j]= data_7[j]/30
        data_8_moy[j]= data_8[j]/30

    for j in range(4):
		data_season[j] = (data_2_moy[j] - data_1_moy[j]) - (data_4_moy[j] - data_3_moy[j]) + (data_6_moy[j] - data_5_moy[j]) - (data_8_moy[j] - data_7_moy[j])
    ...
    for i, ax in enumerate(axes.flat):
        ...
        ax.gridlines(xlocs=xticks, ylocs=yticks,linestyle='--',lw=1,color='dimgrey')
            ...
            # Label the end-points of the gridlines using the custom tick makers:test_diff_rl_all_season.py
        ...
        ax.set_title('Surface CRE {0} ({1}) - [2007-2016] - {2}'.format(data_type, units, title_season[i]))
...
if __name__=="__main__":
    ...
    vmax = args.M
    data_type = args.t
    generate_season_net_surface(fin1, fin2, fin3, fin4, fin5, fin6, fin7, fin8, data_type, vmin, vmax)
